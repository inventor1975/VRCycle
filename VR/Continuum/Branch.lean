...
-- that is inconsistent (classical logic proves a discontinuous `Branch → Nat` exists).
...
-- A `Branch` (`Nat → Bool`) is the *becoming/potential* sequence as a whole; its finite
...
def Branch.take (α : Branch) (n : Nat) : List Bool := (List.range n).map α
...
/-- **The space of branches is non-enumerable.**  No `e : Nat → Branch` is surjective:
...
theorem branches_not_enumerable : ¬ ∃ e : Nat → Branch, Function.Surjective e := by
  rintro ⟨e, he⟩
...
def Continuity : Prop :=
  ∀ (F : Branch → Nat) (α : Branch), ∃ n, ∀ β : Branch, α.take n = β.take n → F α = F β
