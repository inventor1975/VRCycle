...
-- operationally — by a finite-information "associate" `φ : List Bool → Option Nat` that is
...
import VR.Continuum.UniformContinuity
import VR.Prelude.Nat
...
`φ : List Bool → Option Nat` that **decides along every branch** (`bar`).  No modulus and no
...
noncomputable def eval (F : NbhdFun) (α : Branch) : Nat :=
  (F.φ (α.take (VRCycle.Nat.find (F.bar α)))).get (VRCycle.Nat.find_spec (F.bar α))
...
theorem some_eval (F : NbhdFun) (α : Branch) :
    some (F.eval α) = F.φ (α.take (VRCycle.Nat.find (F.bar α))) := by
...
theorem continuity_of_nbhd (F : NbhdFun) (α : Branch) :
  ...
  refine ⟨VRCycle.Nat.find (F.bar α), ?_⟩
  intro β hβ
  -- α and β agree on every prefix up to the deciding depth.
  have hagree : ∀ j, j ≤ VRCycle.Nat.find (F.bar α) → α.take j = β.take j :=
  ...
  have hfindβ : VRCycle.Nat.find (F.bar β) = VRCycle.Nat.find (F.bar α) := by
    apply Nat.le_antisymm
    · apply Nat.le_of_not_lt
      intro hlt
      apply VRCycle.Nat.find_min (F.bar β) hlt
      rw [← hagree _ (Nat.le_refl _)]
      exact VRCycle.Nat.find_spec (F.bar α)
    · apply Nat.le_of_not_lt
      intro hlt
      apply VRCycle.Nat.find_min (F.bar α) hlt
      rw [hagree _ (Nat.le_of_lt hlt)]
      exact VRCycle.Nat.find_spec (F.bar β)
  -- the values agree.
  have hsβ : some (F.eval β) = F.φ (α.take (VRCycle.Nat.find (F.bar α))) := by
    rw [some_eval, hfindβ, hagree _ (Nat.le_refl _)]
  have hsα : some (F.eval α) = F.φ (α.take (VRCycle.Nat.find (F.bar α))) := some_eval F α
