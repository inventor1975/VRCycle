...
--   (2) the BECOMING register (lawless branches `Nat → Bool`, the "potential") is NON-ENUMERABLE
...
-- Classically, Cantor's diagonal proves |℘(Nat)| > |Nat| by selecting an element from the
-- COMPLETED totality ℘(Nat). Here there is no completed totality: the diagonal still cuts, but
...
"done") is enumerated by `decodeNode : Nat → List Bool`, which is surjective (left-inverse of the
hand-rolled `encodeNode`).  Choice-free — the operational register is constructively listable. -/
theorem operational_register_countable : ∃ e : Nat → List Bool, Function.Surjective e :=
...
theorem operational_cantor :
    (∃ e : Nat → List Bool, Function.Surjective e) ∧
...
-- ============================================================
-- §Bridge to ℘(Nat): the diagonal separates describable from completed
-- ============================================================
--
-- A subset of Nat is its characteristic function `Nat → Bool` (= a `Branch`); the space of all
-- such functions IS ℘(Nat).  "Operational / describable family of subsets" = an ENUMERABLE
-- family `e : Nat → (Nat → Bool)`.  The diagonal escapes every such family; the full ℘(Nat) is
-- non-enumerable.  Hence the uncountability of ℘(Nat) rests on the non-describable diagonal
...
-- is complete." That ℘(Nat) as a completed totality is operationally absent is essay-level
-- (meta, VR-LOGIC §3), NOT claimed in the code.

/-- **The diagonal escapes any enumeration of subsets.**  For every countable (describable)
family `e : Nat → (Nat → Bool)` of subsets of Nat, the diagonal subset `d n = !(e n n)` differs from
every `e k`.  Choice-free — Cantor's diagonal, on characteristic functions. -/
theorem diagonal_escapes_enumeration (e : Nat → (Nat → Bool)) :
    ∃ d : Nat → Bool, ∀ k, d ≠ e k := by
  refine ⟨fun n => !(e n n), ?_⟩
  intro k hk
  have h : (!(e k k)) = e k k := congrFun hk k
  cases hb : e k k <;> rw [hb] at h <;> exact Bool.noConfusion h

/-- **℘(Nat), as characteristic functions, is non-enumerable.**  No `e : Nat → (Nat → Bool)` is
surjective — its diagonal subset is missed.  Choice-free. -/
theorem powerset_not_enumerable : ¬ ∃ e : Nat → (Nat → Bool), Function.Surjective e := by
  rintro ⟨e, he⟩
  obtain ⟨d, hd⟩ := diagonal_escapes_enumeration e
  obtain ⟨k, hk⟩ := he d
  exact hd k hk.symm

/-- **The ℘(Nat) bridge.**  The full powerset (characteristic space) is non-enumerable (1), yet
every countable/describable family of subsets misses its own diagonal (2).  So the
uncountability of ℘(Nat) requires the non-describable diagonal subset: it lives in the completed
totality, not in the operational (enumerable) layer.  Machine-checked, choice-free; the
operational absence of the completed ℘(Nat) is essay-level (VR-LOGIC §3), not asserted here. -/
theorem powerset_diagonal :
    (¬ ∃ e : Nat → (Nat → Bool), Function.Surjective e) ∧
    (∀ e : Nat → (Nat → Bool), ∃ d : Nat → Bool, ∀ k, d ≠ e k) :=
