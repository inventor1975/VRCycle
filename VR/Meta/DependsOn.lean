...
EXTERNAL (non-VR) lemma each axiom first arrives. -/
elab "#axiom_census" : command => do
  let env ← getEnv
  let isOurs (m : Name) : Bool := (m.toString.startsWith "VR.") || m.toString == "VR"
...
with the external (non-VR) lemmas it arrives through and the internal ones it inherits it from.
...
elab "#axiom_offenders_all " ax:ident " in " "[" ms:ident,* "]" : command => do
  ...
  let isOurs (mm : Name) : Bool := (mm.toString.startsWith "VR.") || mm.toString == "VR"
  let mut cache : Std.HashMap Name (Array Name) := {}
