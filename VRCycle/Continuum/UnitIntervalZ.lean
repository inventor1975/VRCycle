-- VRCycle/Continuum/UnitIntervalZ.lean — TEMPORARY: the pre-integrity `ℤ` version, kept only
-- until `Real.lean` is rebuilt on VR's own numbers (integrity step 1e); then delete.
