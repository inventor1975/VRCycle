...
--   Every VR object is a TERM over the signature {base (0-ary), succ (1-ary)} —  (∈ is
--   DEFINED by recursion on t, §5; it is not a primitive)
...
--
-- VR has TWO primitive operations: the nullary base ∅ and the unary succession t.
-- (Earlier versions listed a third, the propositional → on a two-valued type VRBool. It
--  has been removed: it generated only a finite truth-function algebra used by no VR
--  theorem — that is arithmetic on a 2-element type, not "logic" in any inferential sense.
--  The logic VR reasons WITH — ∀, →, ↔, induction, Leibnizian equality — is metatheoretic,
--  borrowed from the ambient framework, NOT generated by VR. At this layer VR is pure
--  arithmetic; Theorem_11_VR_PA below makes the content precise: VR ≃ first-order PA.)
...
-- Lean's Iff (↔) is used (metatheoretic equivalence, not a VR-internal operator):
