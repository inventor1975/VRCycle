-- VRCycle.Continuum — the operational continuum (Path 1) and the operational number spectrum.
--
-- Two strands, all below the `Classical.choice` floor (hand-built from ℤ, never mathlib ℚ/ℝ):
--   • the becoming/Brouwerian core (Spread … Model) — non-enumerable operational continuum;
--   • the operational number spectrum ℤ→ℚ→ℂ→ℝ→Ω (UnitInterval, Real, Rational, GaussianRational),
--     consolidated and machine-witnessed in `Spectrum`.
