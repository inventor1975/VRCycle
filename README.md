...
- `VRObj` — inductive type generated by `mark` (∅) and `succ` (t)
...
- `embedVR : VRObj → OSet.{0}` — `mark ↦ ∅`, `succ x ↦ osetSuccOp (embedVR x)`
...
**Ontological theses.** The preprint makes claims about minimalism, the empty set as **Spencer-Brown's mark** (the first distinction — `∅ = {}` as a boundary drawn around the void, an operational act rather than an inert void; the base constructor is named `mark`), and the operational character of objects. These are interpretive layers on top of the formal system. This Lean formalisation verifies formal derivability given a specific translation into Lean types — not the philosophical claims themselves.
