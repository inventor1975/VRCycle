...
## Two libraries: `VR` (VR proper, axiom-free by construction) and `VRClassical`

Since 2026-09-12 the package builds **two Lake libraries**:

| library | what it is | axiom profile |
|---|---|---|
| **`VR`** (directory `VR/`, root `VR.lean`) | VR proper: `VR/VRArithmetic.lean` (the formal system), the witnessed numbers ℤ→ℚ→ℝ on their own pairs (`Numbers/*Op`), the ZTL and operational set universes (`SetsZTL/`, `SetsOp/`), the forms (`Forms/`), the topology tower (`Topology/` up to Tychonoff), the Brouwer continuum (`Continuum/`), the apparatus (`Apparatus/`), the instruments (`Meta/`) | **`[]` for every declaration** — enforced at build time by `VR/Guard.lean` (`#assert_axiom_free_library VR`); `lake build VR` fails otherwise. 2026-09-12: 1516 declarations checked. |
