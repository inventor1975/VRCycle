...
--
-- Split from the VR core on 2026-09-12 (integrity programme). `VR` (the other library of this
-- package) is VR proper: numbers, ZTL/operational sets, forms, topology, continuum, apparatus — every
-- declaration with `#print axioms = []` (guarded at build time by `VR/Guard.lean`).
...
-- over ℝ/PSet. It depends on `VR`; nothing in `VR` depends on it.
